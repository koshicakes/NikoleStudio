...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // groupCover: the shared cover image URL for all packages in this session type.
    // Falls back to the individual package's own cover_image_url if groupCover is absent.
    const renderDbPackage = (group, pkg, groupCover) => `
    ...
    mount.innerHTML = groups.map(group => {
        ...
        // Resolve the shared cover for this session type:
        // use the first non-empty cover_image_url found among the group's DB packages.

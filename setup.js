...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Split into backdrops (asset_type = 'background') and props (everything else)
    ...
    if (dbAssets && dbAssets.length > 0) {
        ...
        // Fall back to static if a category has nothing
    ...
    function removeBackdrop() {
        if (backdropImg) { canvas.remove(backdropImg); backdropImg = null; selectedBackdropName = ''; canvas.renderAll(); saveHistory(); showToast('Backdrop removed'); }
    }
    ...
    function deleteSelected() {
        ...
        canvas.remove(obj); canvas.discardActiveObject(); canvas.renderAll(); saveHistory(); updateSelectionPanel(); showToast('Removed');
    }
    ...
    function duplicateSelected(offset = 22) {
        ...
        obj.clone(clone => {
            ...
            tuneObject(clone); canvas.add(clone); canvas.setActiveObject(clone); canvas.renderAll(); saveHistory(); showToast('Duplicated');
        });
    }
    ...
    function clearProps() {
        ...
        canvas.discardActiveObject(); canvas.renderAll(); saveHistory(); updateSelectionPanel(); showToast('Props cleared');
    }
    ...
    function undo() {
        ...
        canvas.loadFromJSON(history[history.length - 1], () => {
            canvas.renderAll(); restoreBackdropReference(); updateSelectionPanel(); restoringHistory = false;
        });
    }
    ...
    function bindEvents() {
        ...
        canvas.on('object:modified', () => {
            ...
            saveHistory(); updateSelectionPanel();
        });
        canvas.on('mouse:down', e => {
            const obj = e.target;
            if (!obj || PROTECTED.includes(obj.name) || movingShadows.has(obj)) return;
            movingShadows.set(obj, obj.shadow || null); obj.set('shadow', null); canvas.requestRenderAll();
        });
        canvas.on('mouse:up', () => {
            canvas.getObjects().forEach(obj => {
                if (!movingShadows.has(obj)) return;
                obj.set('shadow', movingShadows.get(obj)); movingShadows.delete(obj);
            });
        ...
        canvas.on('mouse:down', e => {
            ...
            const obj = selectedObject();
            if (obj && !PROTECTED.includes(obj.name)) duplicateSelected(20);
        });
